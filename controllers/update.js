...
module.exports = {
    ...
    admin_terminate_interview: function(request,response){
        if( (request.body.user_id!=undefined) && (request.body.user_id!="") && (request.body.interview_id!=undefined) && (request.body.interview_id!="") && (request.body.user_email!=undefined) && (request.body.user_email!="") && (request.body.startup_id!=undefined) && (request.body.startup_id!="")){
            Sessions.validate(request.params.session_id,request.body.user_id,function(validated){
                if(validated){
    ...
    task_status: function(request,response){
        if((request.body.user_id!=undefined) && (request.body.user_id!="") && (request.body.team_id!=undefined) && (request.body.team_id!="") && (request.body.status!=undefined) && (request.body.status!="") && (request.body.task_id!=undefined) && (request.body.task_id!="")){
            Sessions.validate(request.params.session_id,request.body.user_id,function(validated){
                if(validated){
                    TeamMembers.validate_membership(request.body.team_id,request.body.user_id,function(member){
                        if(member){
                            Tasks.update_task_status(request.body,response);
                        }else{
                            response.data = {};
                            response.writeHead(201,{'Content-Type' : 'application/json'});//server response is in json format
