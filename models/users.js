...
exports.fetch_user_invites = function(user_email,response){
    ...
    var aggregate = [{
        ...
        $lookup: {
            from: "personnel_queue",
            foreignField: "personnel_email",
            localField: "email",
            as: "job_invites" //include code to hide data if accepted or rejected is true
        }
    },{
        $lookup: {
            from: "founders_invites",
        ...
        $lookup: {
            from: "privilegesqueue",
            foreignField: "user_email",
            localField: "email",
            as: "privileges_invites"
        }
    },{
        $project: {
            id: 0,
            ...
            updated_at: 0,
            created_at: 0,            
            personnel_invites_num: {$size: personnel_invites},
            founder_invites_num: {$size: founder_invites},
            privileges_invites_num: {$size: privileges_invites},
            personnel_invites: 1,
            founder_invites: 1,
            privileges_invites: 1
        }
    }]
    
    Users.aggregate(aggregate,function(error,data){
        if(error){
            if(response==null){
                response.writeHead(500,{'Content-Type':'application/json'});
				response.data.log = "Internal server error";
